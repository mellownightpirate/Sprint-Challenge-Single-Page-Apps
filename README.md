...
> Answer: Client-side routing allows for dynamic single-page web apps with navigation that doesn't require the page to refresh, thus preventing a request to the server. This means the user doesn't have to wait for a response, resulting in a change of state of the application which uses a lot less memory and data and ultimately, allows for a much faster and more efficient user experience.
